...
class MainWindow(QMainWindow):
    ...
    def create_movies_tab(self):
        ...
        layout.addWidget(instructions)
        drop_area = QListWidget()
        drop_area.setAcceptDrops(True)
        drop_area.setDragDropMode(QAbstractItemView.DropOnly)
        ...
        def sort_movies():
            ...
            for i in range(drop_area.count()):
                file_path = Path(drop_area.item(i).text())
                if file_path.exists() and file_path.is_file():
                    movie_handler.sort_movie(file_path, movie_folder)
            self.log_action("Movie sorting complete.")

        drop_area.dropEvent = handle_drop
        drop_area.dragEnterEvent = drag_enter
