# Movie Handler for Sorting Logic
...
# GUI logic for Movies tab
def create_movies_tab(self):
    tab = QWidget()
    layout = QVBoxLayout()
    tab.setLayout(layout)

    instruction_label = QLabel("Drag and drop movies here or use the 'Add Movies' button.")
    layout.addWidget(instruction_label)

    self.movie_list_widget = QListWidget()
    self.movie_list_widget.setAcceptDrops(True)
    self.movie_list_widget.setDragDropMode(QAbstractItemView.DropOnly)
    layout.addWidget(self.movie_list_widget)

    self.movie_progress_bar = QProgressBar()
    self.movie_progress_bar.setValue(0)
    layout.addWidget(self.movie_progress_bar)

    add_movies_button = QPushButton("Add Movies")
    add_movies_button.clicked.connect(self.add_movie_files)
    layout.addWidget(add_movies_button)

    sort_movies_button = QPushButton("Sort Now")
    sort_movies_button.clicked.connect(self.sort_movie_files)
    layout.addWidget(sort_movies_button)
