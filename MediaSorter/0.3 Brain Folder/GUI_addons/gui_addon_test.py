...
def add_movie_sorting_tab(main_window):
    ...
    def sort_movies():
        """Sort movies using movie_handler logic."""
        from movie_code import movie_handler  # Import movie_handler
        movie_folder = main_window.main_folder / "0.1 Sorting Folder"  # Use sorting folder
        movies_folder = main_window.main_folder / "Movies"

        for file in movie_folder.iterdir():
            if file.is_file():
                movie_handler.sort_movie(file, movies_folder)
